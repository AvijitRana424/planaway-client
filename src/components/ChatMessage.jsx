  ...
  const renderInline = (text) => {
    ...
    return parts.map((p, i) =>
      p.startsWith("**") && p.endsWith("**")
        ? <strong key={i} style={{ color: 'var(--text)', fontWeight: 500 }}>{p.slice(2, -2)}</strong>
        : p.startsWith("*") && p.endsWith("*")
...
const styles = {
  ...
  botBubble: { background: 'var(--card)', border: '1px solid var(--border)', borderTopLeftRadius: 4 },
  userBubble: { background: 'var(--sky-dim)', border: '1px solid rgba(74,158,255,0.25)', borderTopRightRadius: 4, color: 'var(--text)' },
  h2: { fontFamily: 'Playfair Display, serif', fontSize: 17, fontWeight: 600, color: 'var(--text)', marginBottom: 8, marginTop: 6 },
  h3: { fontFamily: 'Playfair Display, serif', fontSize: 15, fontWeight: 600, color: 'var(--gold)', marginBottom: 6, marginTop: 8 },
  para: { fontSize: 14, color: 'var(--text)', lineHeight: 1.65, margin: 0 },
